...
#              echo "cp $zara_files_path/get_cfsr_zara.sh $tmp_work_dir" >> $tmp_script
              if [ $flag_reprocess_l2_files -eq 0 ] ; then 
                 echo "cp $zara_files_path/check_filelist_zara.sh $tmp_work_dir" >> $tmp_script
              fi
              echo "cp $zara_files_path/$clavrx_run_file $tmp_work_dir" >> $tmp_script
...
#              echo "./get_cfsr_zara.sh $year $doy" >> $tmp_script
      ...
      if [ $flag_make_l2b -ne 0 ] ; then
         ...
         echo "cp $zara_files_path/$clavrx_l2b_run_file $tmp_work_dir_l2b" >> $tmp_script_l2b
         echo "cp $zara_files_path/level2b_script.sh $tmp_work_dir_l2b" >> $tmp_script_l2b
