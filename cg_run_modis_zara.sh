   ...
   # loop hours
	...
	do
		...
		echo "[ ! -d $tmp_work_dir/clavrx_ancil_data ] && ln -s /fjord/jgs/patmosx/Ancil_Data/clavrx_ancil_data $tmp_work_dir" >> $tmp_script
   		...
   		echo "./cg_get_modis_data.sh --path $l1b_path --reg $region $year$doy_str $hhh " >> $tmp_script
   		...
   		echo "./write_filelist.sh $l1b_path $out_path $filetype $year$dd"."${hhh_str}" >> $tmp_script
