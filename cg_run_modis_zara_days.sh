...
do
   ...
   echo 'Processing DOY: ' $doy_str,$month$day
	...
	tmp_script=$work_dir'modis_andi'$year'_'$doy_str'_'$region'_patmosx.sh'
   tmp_work_dir=$work_dir'modis_andi'$year'_'$doy_str'_'$region
   ...
   echo "echo 'Processing viirs $year $doy_str  $region'" >> $tmp_script
   ...
   echo "./cg_get_modis_data.sh --path $l1b_path --reg $region $year$doy_str 00 23 " >> $tmp_script
   echo "echo 'Making sure all files are there, running sync_viirs_zara.sh'" >> $tmp_script

   echo "./write_filelist.sh $l1b_path $out_path $filetype $year$dd"."" >> $tmp_script
